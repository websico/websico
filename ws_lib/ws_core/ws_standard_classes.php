...
class WssTitle extends WsText {
	...
	function __construct($value='', $level=2) {
...
class WssForm extends WsContainer {
	...
	function __construct($user_style=0, $properties = 0, $contents = 0){
        if (!$contents)
            $contents = array(
        ...
        parent::__construct(0, 0, $contents);
	...
	function Display($html_prefix = '', $html_suffix = '') {
        ...
        $prefix = '<form ';
        // If destination is a javascript, it's the action
        if (stripos($this->destination, 'javascript:') === 0)
            $prefix .= 'action="'.$this->destination.'" ';
    ...
    function Action() {
		...
		// and do some sanitary controls on the form
        ...
        if (!empty($this->destination)
				...
				&& !$ws_insecure
				&& !(empty($this->ccPrompt) && !empty($_POST['ws_ccEmail']))) {
	  	    $mailText = '';
        ...
        } else {    // SPAM or robot (or both:), invalid form submission
			exit;
		}
...
class WssInputField extends WsText {   // Can be out of a form, so it's not named FormField..
    ...
    function __construct($caption = '', $type = 'text', $required = false, $params = 0, $htmlName = '') {
...
class WssRSSReader extends WsComponent {
		...
		require_once(WS_LIB_PATH.'third_party_includes/ws_simplepie.inc');
	  	if (!empty($this->value) && ($feed = new SimplePie())){
			...
			if ($this->display_channel) {
				...
				if (!($this->properties & WS_IN_BLOCK_LINK)){
					$prefix = '<a href="'.$feed->get_permalink().'"'.$this->buildTarget($feed->get_permalink()).'>';
					$suffix = '</a>';
				}
            ...
            foreach ($feed->get_items(0, $this->max_items) as $item) {
                echo '<article><header>';
				if (!($this->properties & WS_IN_BLOCK_LINK)){
					$prefix = '<a class="itemTitle" href="'.$item->get_permalink().'"'.$this->buildTarget($item->get_permalink()).'>';
					$suffix = '</a>';
				}
	...
	private function buildTarget($link){
        if (preg_match("@^htt.*//".$_SERVER['SERVER_NAME']."@", $link))
			return "";
		else
...
class WssField {
    ...
    function __construct($caption, $lines, $notempty) {
