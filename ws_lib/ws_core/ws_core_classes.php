...
//	Constructor
//	-----------
	function __construct($id=0, $properties=0, $user_style='') {
...
//  Constructor
//  -----------
    function __construct($css_id = 'default', $purge = false) {
	  	global $ws_permissions;
...
//	Constructor
//	-----------
	function __construct($properties=0, $value=0) {
...
class WsText extends WsComponent {
	function __construct($value='') {
		parent::__construct(WS_TEXT, $value);
	}
...
class WsTextarea extends WsComponent {
	function __construct($value='') {
		parent::__construct(WS_TEXTAREA, $value);
	}
...
class WsRawText extends WsComponent {
	...
	function __construct($value = '', $source_filename = 0) {
		parent::__construct(WS_RAWTEXT, $value);
...
class WsMapItem {  // A map item is a page with or without children
    ...
    function __construct($id, $url_id, $name, $status = 0) {
        $this->id = $id;
        $this->url_id = $url_id;
        $this->name = $name;
        $this->status = $status;
	...
	function __construct($map = 0) {
		...
		foreach($map_array as $key => $page) {
			if (array_key_exists($page['id'], $page_list)) {     // Check page existence
				...
				$newchild = new WsMapItem(	$page['id'],                // id
		                                    $url_id[$page['id']],       // url_id
		                                    $page_list[$page['id']],    // name
		                                    $status[$page['id']]);		// status
                $this->item_by_id[$page['id']] = $current_cat->AddChild($newchild);
				unset($not_seen[$page['id']]);
			}
		}
		foreach($not_seen as $id => $name){					// Add pages not yet in map
			$newchild = new WsMapItem($id, $url_id[$id], $name, $status[$id]);
            $this->item_by_id[$id] = $this->root->AddChild($newchild);
		}
		$ws_sitemap = &$this;
    }
